...
DATABASE_PATH = ROOT / "data" / "adbs_exam_prep.sqlite3"
...
def database_connection(path: Path = DATABASE_PATH) -> sqlite3.Connection:
...
def _decode_json_rows(
    rows: list[sqlite3.Row], key_column: str, payload_column: str = "payload"
) -> dict[str, Any]:
    return {
        row[key_column]: json.loads(row[payload_column])
        for row in rows
    }


def load_user_state(path: Path = DATABASE_PATH) -> dict[str, Any]:
    ...
    try:
        ...
        progress_rows = list(
            connection.execute(
                "SELECT question_id, payload FROM progress ORDER BY question_id"
            )
        )
        override_rows = list(
            connection.execute(
                "SELECT question_id, payload FROM question_overrides ORDER BY question_id"
            )
        )
        custom_rows = list(
            connection.execute(
    ...
    return {
        ...
        "progress": _decode_json_rows(progress_rows, "question_id"),
        "questionOverrides": _decode_json_rows(override_rows, "question_id"),
        "customQuestions": [
            json.loads(row["payload"]) for row in custom_rows
        ],
        "examHistory": [
            json.loads(row["payload"]) for row in history_rows
        ],
    }


def validate_user_state(payload: Any) -> dict[str, Any]:
    if not isinstance(payload, dict):
        raise ValueError("Der gespeicherte Zustand muss ein JSON-Objekt sein.")

    progress = payload.get("progress", {})
    overrides = payload.get("questionOverrides", {})
    custom_questions = payload.get("customQuestions", [])
    exam_history = payload.get("examHistory", [])
    language = payload.get("language", "de")

    if not isinstance(progress, dict):
        raise ValueError("progress muss ein Objekt sein.")
    if not isinstance(overrides, dict):
        raise ValueError("questionOverrides muss ein Objekt sein.")
    if not isinstance(custom_questions, list):
        raise ValueError("customQuestions muss ein Array sein.")
    if not isinstance(exam_history, list):
        raise ValueError("examHistory muss ein Array sein.")
    if language not in {"de", "en"}:
        raise ValueError("language muss 'de' oder 'en' sein.")

    topic_ids = {
        topic["id"] for topic in read_json(CONTENT_DIR / "topics.json")
    }
    custom_ids: set[str] = set()
    normalized_custom: list[dict[str, Any]] = []
    for question in custom_questions:
        if not isinstance(question, dict):
            raise ValueError("Jede importierte Frage muss ein Objekt sein.")
        validate_question(question, topic_ids)
        if question["id"] in custom_ids:
            raise ValueError(
                f"Doppelte importierte Frage-ID: {question['id']}"
            )
        custom_ids.add(question["id"])
        normalized_custom.append(question)

    normalized_overrides: dict[str, Any] = {}
    for question_id, override in overrides.items():
        if not isinstance(question_id, str) or not question_id:
            raise ValueError("Override-IDs müssen nichtleere Strings sein.")
        if not isinstance(override, dict):
            raise ValueError(f"{question_id}: Override muss ein Objekt sein.")
        status = override.get("status", "active")
        if status not in STATUS_VALUES:
            raise ValueError(f"{question_id}: ungültiger Status '{status}'.")
        normalized_overrides[question_id] = override

    normalized_history: list[dict[str, Any]] = []
    history_ids: set[str] = set()
    for entry in exam_history[:MAX_EXAM_HISTORY]:
        if (
            ...
            raise ValueError("Ungültiger Eintrag im Prüfungsverlauf.")
    ...
    return {
        "language": language,
        "progress": progress,
        "questionOverrides": normalized_overrides,
        "customQuestions": normalized_custom,
        "examHistory": normalized_history,
    }


def save_user_state(
    payload: Any, path: Path = DATABASE_PATH
) -> dict[str, Any]:
    state = validate_user_state(payload)
    initialize_database(path)
    connection = database_connection(path)
    try:
        with connection:
            ...
            connection.executemany(
                "INSERT INTO progress(question_id, payload) VALUES (?, ?)",
                [
                    (
                        question_id,
                        json.dumps(
            ...
            connection.execute("DELETE FROM question_overrides")
            connection.executemany(
                """
                INSERT INTO question_overrides(
                    question_id, status, changed_at, payload
                ) VALUES (?, ?, ?, ?)
                """,
                [
                    (
            ...
            connection.execute("DELETE FROM custom_questions")
            connection.executemany(
                """
                INSERT INTO custom_questions(
                    question_id, imported_at, payload
                ) VALUES (?, ?, ?)
                """,
                [
                    (
            ...
            connection.execute("DELETE FROM exam_history")
            connection.executemany(
                """
                INSERT INTO exam_history(exam_id, finished_at, payload)
                VALUES (?, ?, ?)
                """,
                [
                    (
...
def validate_question(question: dict[str, Any], topic_ids: set[str]) -> None:
    ...
    if question.get("status", "active") not in STATUS_VALUES:
        raise ValueError(f"{question_id}: status muss active, archived oder deleted sein")
    if not all(
...
BEGINNER_EXPLANATIONS.update(
    {
        ...
        "WHERE": multilingual(
            "WHERE filtert bereits gefundene Cypher-Ergebnisse, etwa auf Personen mit age > 18 oder auf Knoten mit einer bestimmten Property.",
            "WHERE filters Cypher matches, for example people with age > 18 or nodes having a certain property.",
        ),
        "RETURN": multilingual(
    ...
    for path in sorted(QUESTION_DIR.glob("*.json")):
        try:
            ...
            for question in entries:
                ...
                question["_languages"] = question.get("languages", ["de"])
...
class AppHandler(SimpleHTTPRequestHandler):
    ...
    def do_GET(self) -> None:
        ...
        if parsed.path == "/api/state":
            self.send_json(load_user_state())
            return
    ...
    def do_PUT(self) -> None:
        ...
        try:
            ...
            if content_length <= 0:
                raise ValueError("Leerer Request-Body.")
            ...
            self.send_json(save_user_state(payload))
        except (ValueError, json.JSONDecodeError) as exc:
            self.send_json({"error": str(exc)}, HTTPStatus.BAD_REQUEST)
        except sqlite3.Error as exc:
            self.send_json(
                {"error": f"Datenbankfehler: {exc}"},
    ...
    def log_message(self, format: str, *args: object) -> None:
        print(f"[ADBS] {self.address_string()} - {format % args}")
...
def main() -> None:
    ...
    print(f"ADBS Exam Prep läuft auf http://{host}:{port}")
    print(f"Persistente Datenbank: {DATABASE_PATH.relative_to(ROOT)}")
